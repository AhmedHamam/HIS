...
namespace HIS
{
    public partial class returnFromEmployee : Form
    {
        SqlConnection con = new SqlConnection(@"Server=DESKTOP-O2AL8TR\SQLEXPRESS;Database=HIS_GROUP_J; Integrated Security=true");
        public returnFromEmployee()
        {
            InitializeComponent();
        }

        private void returnFromEmployee_Load(object sender, EventArgs e)
        {

            try
            {
                con.Open();
                SqlCommand cmd = new SqlCommand("select * from Inventory_inventoryOperation where type='return_from_employee' ", con);
                SqlDataReader dr;
