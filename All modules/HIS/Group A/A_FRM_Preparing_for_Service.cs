...
namespace HIS
{
     public partial class A_FRM_Preparing_for_Service : Form
    {
        public A_FRM_Preparing_for_Service()
        {
            InitializeComponent();
        }
        ...
        {
            con.OpenConection();
            ds = con.select("select * from tb_Preparing_for_Service ;", "tb_Preparing_for_Service");
        ...
        private void C_Save_Click(object sender, EventArgs e)
        {
            ...
            if (check())
            {
                if (con.update(ds, "tb_Preparing_for_Service"))
                {
