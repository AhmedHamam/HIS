...
namespace HIS
{
    public partial class A_FRM_Operation_Skills_Desc : Form
    {
        public A_FRM_Operation_Skills_Desc()
        {
            InitializeComponent();
        }
        ...
        private void A_FRM_Operation_Skills_Desc_Load(object sender, EventArgs e)
        {
            
            con.OpenConection();
            ds = con.select("select * from tb_Operations_Skills_Description;", "tb_Operations_Skills_Description");
            if (view)
            {
                dt = ds.Tables[0];
                DGV.DataSource = dt;
                ...
                DGV.Columns[3].HeaderText = "التوصيف الاساسى";
            }
            else
            {
                dt = ds.Tables[0];
        ...
        private bool check()
        {
            ...
            for (int i = 0; i < DGV.Rows.Count; i++)
            {
                ...
                if (Convert.ToBoolean(DGV.Rows[i].Cells[3].Value) == true)
                {
                    cc++;
                }
                ...
                if (cc > 1)
                {
                    MessageBox.Show("يجب اختيار توصيف اساسى واحد ");
            ...
            if (cc == 0)
            {
                MessageBox.Show("يجب اختيار توصيف اساسى");
                return false;
            }
            return true;
        }
        ...
        private void C_Add_Click(object sender, EventArgs e)
        {
            ...
            {                
                ...
                DGV.Rows[DGV.Rows.Count - 1].Cells[DGV.Columns.Count - 1].Value = false;
