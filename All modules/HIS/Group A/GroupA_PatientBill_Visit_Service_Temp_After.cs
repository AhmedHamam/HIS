...
namespace HIS
{
    ...
    public partial class GroupA_PatientBill_Visit_Service_Temp_After : UserControl
    {
        int User_ID;
        Connection conn;
        SqlDataReader dr;
        ...
        public GroupA_PatientBill_Visit_Service_Temp_After(int uid)
        {
            InitializeComponent();
            conn = new Connection();
            User_ID = uid;
       ...
       private void calculate()
        {
            try
            {
                ...
                label3.Text = (discount_percentage * total1 / 100).ToString();
            }
            catch (Exception ex)
            {
        ...
        private void get_percentage()
        {
            try
            {
                query = @"SELECT (patient_service_amount/total_service*100) FROM  Visit_Bill WHERE visit_ID=" + VISIT_ID + "";
            ...
            if (e.ColumnIndex == 5)
            {
                ...
                if (dialogResult == DialogResult.Yes)
                {
                    try
                    {
                        ...
                        query = "UPDATE Visit_Bill SET total_service=" + label11.Text + ", patient_service_amount=" + label3.Text + " WHERE visit_ID=" + VISIT_ID + "";
                        conn.ExecuteQueries(query);
